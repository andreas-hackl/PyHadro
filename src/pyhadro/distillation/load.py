...
def load_single_eps(fn, N_dist=None):
    d_eps = lqcdpy.gpt_io.get_data(fn+'/head.dat')
...
def load_eps(src, momenta, N_dist=None):
    print(f"{lqcdpy.timer.__str__()}Loading eps tensors", flush=True)
...
def load_single_peramb(fn, N_dist=None):
    d_peramb = lqcdpy.gpt_io.get_data(fn+'/head.dat')
...
def load_peramb(src, t0s, prec, N_dist=None):
    print(f"{lqcdpy.timer.__str__()}Loading perambulators", flush=True)
...
def load_mins(fn, N_dist=None):
    print(f"{lqcdpy.timer.__str__()}Loading momentum insertions", flush=True)
    d_mins = lqcdpy.gpt_io.get_data(fn+'/head.dat')
