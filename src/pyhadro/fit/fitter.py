...
class Fitter:
    ...
    def fitting_lm(self, y, maxiter=1000, fjac=None):
        sol = pyhadro.fit.optimizer.levenberg_marquardt(self.model, self.p0, y, self.W, maxiter=maxiter, fjac=fjac)
        if not sol.success:
            print(sol)
            raise RuntimeError()
        return sol.x
    ...
    def extrapolation_check(self):
        mean, std = pyhadro.fit.extrapolation_check(self.db, self.func, self.in_tag, self.out_tag, 
